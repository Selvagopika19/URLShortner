...
export default function Signup() {
  ...
  async function handleSubmit(ev) {
    ...
    try {
      ...
      setApiError(err?.response?.data?.message ?? 'Could not create account. Try again.');
    } finally {
      setLoading(false);
    }
  }
  ...
  const field = (id, label, type, placeholder, autocomplete) => {
    ...
    return (
      <div className="space-y-1.5">
        <label htmlFor={id} className="snip-label">{label}</label>
        <div className="relative">
          <input
            id={id}
            type={isPass ? (showPass ? 'text' : 'password') : type}
            autoComplete={autocomplete}
            className={`snip-input pr-10 ${errors[id] ? 'border-danger/60 focus:border-danger/80 focus:ring-danger/15' : ''}`}
    ...
    <div className="flex min-h-screen items-center justify-center px-4 py-12" style={{ backgroundColor: '#0d0d0d', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="w-full max-w-sm animate-fade-up">
        ...
        <div className="mb-8 flex flex-col items-center gap-3 cursor-pointer" onClick={() => navigate('/')}>
          ...
          <p className="text-sm text-slate-500">Start shortening links for free</p>
        </div>
        ...
        <div className="snip-card p-6 bg-white/[0.03] border-white/[0.07]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            {field('password', 'Password',  'password', 'Min 6 characters',  'new-password')}
